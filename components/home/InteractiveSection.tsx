...
const categoryStyles: { [key: string]: { bg: string; text: string; border: string; } } = {
    "진단 & 분석": { bg: 'bg-diagnosis-blue/10', text: 'text-diagnosis-blue', border: 'border-diagnosis-blue' },
...
const allSolutions: Solution[] = [
    { id: 1, title: '차세대 리더 파이프라인 구축', description: '미래 성장을 이끌 핵심 리더를 조기에 발굴하고 체계적으로 육성하는 로드맵을 설계합니다.', q_categories: ['Q1'], solution_category: '전략 컨설팅' },
    { id: 2, title: '임원 리더십 코칭', description: '고위 리더의 전략적 통찰력과 조직관리 역량을 극대화하는 1:1 맞춤 코칭을 제공합니다.', q_categories: ['Q1'], solution_category: '역량 개발' },
    { id: 3, title: '애자일 조직구조 설계', description: '시장 변화에 민첩하게 대응하고 부서 간 협업을 촉진하는 유연한 조직구조를 디자인합니다.', q_categories: ['Q2'], solution_category: '전략 컨설팅' },
    { id: 4, title: 'R&R(역할과 책임) 재정의', description: '조직 목표 달성을 위한 명확한 역할과 책임을 정의하고 중복/누락 업무를 제거합니다.', q_categories: ['Q2', 'Q5'], solution_category: '전략 컨설팅' },
    { id: 5, title: '핵심인재 정의 및 관리', description: '우리 조직의 성과를 견인하는 핵심인재를 정의하고, 이들의 유지 및 성장을 지원합니다.', q_categories: ['Q3'], solution_category: '경영지원 (Growth OS)' },
    { id: 6, title: '직무역량 모델링 및 진단', description: '직무별 필요역량을 체계적으로 도출하고, 구성원 역량 수준을 진단하여 육성 계획을 수립합니다.', q_categories: ['Q3'], solution_category: '진단 & 분석' },
    { id: 7, title: '조직문화 진단 서베이', description: '데이터 기반 서베이를 통해 우리 조직문화의 강점과 약점을 객관적으로 진단합니다.', q_categories: ['Q4'], solution_category: '진단 & 분석' },
    { id: 8, title: '수평적 조직문화 구축 워크샵', description: '심리적 안정감을 바탕으로 자유로운 의견 교환과 빠른 실행을 촉진하는 문화를 만듭니다.', q_categories: ['Q4'], solution_category: '역량 개발' },
    { id: 9, title: 'OKR 기반 성과관리 시스템 도입', description: '전사 목표와 팀, 개인의 목표를 정렬하여 성과를 극대화하는 OKR 시스템 구축을 지원합니다.', q_categories: ['Q5'], solution_category: '경영지원 (Growth OS)' },
    { id: 10, title: '성과 피드백 시스템 설계', description: '일회성 평가가 아닌, 상시적이고 건설적인 피드백이 오가는 시스템을 설계합니다.', q_categories: ['Q5', 'Q4'], solution_category: '역량 개발' },
];

const qFilters: { key: string; label: string }[] = [
    { key: 'All', label: '전체보기' },
    { key: 'Q1', label: 'Q1: 리더십' },
    { key: 'Q2', label: 'Q2: 조직구조' },
    { key: 'Q3', label: 'Q3: 인재와 역량' },
    { key: 'Q4', label: 'Q4: 문화와 몰입' },
    { key: 'Q5', label: 'Q5: 성과관리' },
];

const solutionCategories: SolutionCategory[] = ["진단 & 분석", "전략 컨설팅", "역량 개발", "경영지원 (Growth OS)"];
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                         <div className="flex flex-wrap justify-center md:justify-start gap-2">
                            {qFilters.map(item => (
                                <button
                                    ...
                                    className={`px-5 py-2.5 text-body-sm font-semibold rounded-full transition-colors duration-300 leading-none ${activeQ === item.key ? 'bg-strategy-blue text-white' : 'bg-gray-100 text-text-secondary hover:bg-gray-200'}`}
                                >
                                    {item.label}
                                </button>
                            ))}
                        ...
                        <div className="relative w-full md:w-auto md:min-w-[320px]">
                            <input 
                                ...
                                className="w-full pl-5 pr-12 py-3 border border-border-light rounded-full focus:outline-none focus:ring-2 focus:ring-strategy-blue text-body-base"
                            />
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2 mb-3">
                                                        {solution.q_categories.map(q => (
                                                             <span key={q} className={`text-caption py-1 px-3 rounded-full font-semibold ${activeQ === q ? 'bg-strategy-blue/20 text-strategy-blue' : 'bg-gray-100 text-text-secondary'}`}>{q}</span>
                                                        ))}
                                                    </div>
                                                    <h4 className="text-h5 font-bold text-text-primary mt-2 mb-3 leading-tight tracking-tight-title">{solution.title}</h4>
                                                    <p className="text-body-sm text-text-secondary mb-4 leading-relaxed">{solution.description}</p>
                                                </div>
                                                <button
                                                    onClick={() => togglePortfolio(solution)}
                                                    className={`w-full mt-4 px-4 py-2.5 text-body-base font-semibold rounded-full transition-all duration-300 flex items-center justify-center gap-2 leading-none ${isInPortfolio ? 'bg-white text-error border-2 border-error hover:bg-red-50' : 'bg-strategy-blue text-white border-2 border-transparent hover:bg-opacity-90'}`}
                                                >
                                                    {isInPortfolio 
                                                        ? <><span className="text-xl leading-none">-</span> 제거하기</> 
                                                        : <><span className="text-xl leading-none">+</span> 문의함에 추가</>
                                                    }
                                                </button>
                                            </div>
                                        )
