...
const solutionAreas = [
  ...
  },
  {
    ...
    examples: ['가치체계(M.V.C) 수립', '조직구조 설계', '전략적 인력계획', '직무 분석 및 역량 모델링'],
...
const SolutionCard = ({ engTitle, mainTitle, coreValue, examples, icon }: { engTitle: string, mainTitle: string, coreValue: string, examples: string[], icon: React.ReactNode }) => {
    return (
        <div className="bg-white p-8 rounded-2xl flex flex-col h-full soft-shadow-md hover:soft-shadow-lg hover:-translate-y-1.5 transition-all duration-300 text-left">
          ...
          <div>
            ...
            <p className="text-body-base text-text-secondary leading-relaxed break-keep">{coreValue}</p>
          </div>
