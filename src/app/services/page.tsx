...
const ServiceCard: React.FC<{ title: string; description: string; icon: React.ReactNode; }> = ({ title, description, icon }) => (
    <div className="bg-white rounded-2xl p-8 h-full soft-shadow-md hover:soft-shadow-lg transition-shadow duration-300">
        ...
        <h3 className="text-h4 font-bold text-text-primary mb-3 text-center">{title}</h3>
        <p className="text-body-base text-text-secondary leading-relaxed break-keep text-left">{description}</p>
    </div>
);
            ...
            <Container>
                <div className="text-center max-w-4xl mx-auto">
                    ...
                    <p className="text-body-lg text-text-secondary leading-relaxed mb-16">
                        APX의 BPO 서비스는 단순한 업무 대행이 아닙니다. 법인설립, 재무, 회계, 운영 등 스타트업 성장에 필수적인 경영 인프라를 하나의 통합된 시스템으로 제공하여, 대표님과 팀이 제품 개발과 고객 확보라는 가장 중요한 본질에만 집중할 수 있는 환경을 만듭니다.
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div id="cfo-service" className="scroll-mt-[140px]">
                        <ServiceCard
                            title="CFO 구독 서비스"
                    ...
                    <div id="coo-service" className="scroll-mt-[140px]">
                        <ServiceCard
                            title="COO 구독 서비스"
                    ...
                    <div id="package-service" className="scroll-mt-[140px]">
                         <ServiceCard
                            title="경영지원 패키지"
                ...
                <div className="mt-16 text-center">
                    <button
                        onClick={() => openContactModal('BPO 서비스 문의')}
...
export default function ServicesPage() {
  return (
    <>
      <PageHeader
        engTitle="Business Process Outsourcing"
        title="BPO 서비스"
        description={
          <>
            APX의 BPO 서비스는 스타트업이 성장에만 집중할 수 있도록,
            <br />
            재무부터 운영까지 모든 경영 인프라를 책임지는 파트너입니다.
          </>
        }
      />
      <GrowthOSPageContent />
